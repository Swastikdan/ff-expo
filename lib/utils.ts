...
export function isValidId(id: number): boolean {
  return Number.isInteger(id) && id >= VALID_ID_RANGE.min && id <= VALID_ID_RANGE.max;
}
...
export function parseAndValidateId(input: string | number): ValidationResult<number> {
...
export function safeValidateResponse<T>(response: ApiResponse<T>): ValidationResult<T> {
  try {
